...
Fig.~\ref{DFSi} shows parts that are later compared with other search 
...
with $^\infty$. The first function, \texttt{disj2}, implements binary 
disjunction. if and only if is its helper, which composes two disjunctive 
search spaces. The following function, \texttt{conj2}, implements binary 
conjunction. 
...
\texttt{append$^\infty$} for search space composition. The following and last 
two definitions say disjunctive operator and conjunctive operator are 
right-associative.
...
The first ``argument'' to \texttt{disj+} is the goals. And the next two 
``arguments'' accumulate goals to be put in the left and right subtrees. When 
there is only one goal, the tree is the goal itself. This case is handled in 
the first clause. When there are more goals, we partition the list of goals 
into two sublists of roughly equal lengths and recur on the two sublists. The 
partition is done by repetitively dispatching the first two goals in the list 
to the accumulators, until only 
zero or one goal remains. The two base cases of the partition are handled by 
the second and the third clauses respectively. And the dispatching is done at 
the fourth and last clause. In contrast, the \disj{} in \DFSi{} constructs the 
