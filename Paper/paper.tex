...
unexpected result. With this search strategy, each clause takes a half 
...
A \disj{} is almost-fair iff it allocates computational resource so evenly to 
...
constructs the binary tree with the same nodes but in an unbalanced form.
...
\texttt{append-inf/fair} immediately calls its helper, \texttt{loop}, with the 
first argument, \texttt{s?}, set to \texttt{\#{}t}, which indicates that 
...
answers). We swapped them back so that answers are produced in a more natural 
order.
