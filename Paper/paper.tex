...
different states. These states are then supplied individually to 
\texttt{(repeato x q)} goal, producing four search spaces. In the examples 
above, the \conj{}s are allocating more computational resources to the state of 
\texttt{a}. On the contrary, fair \conj{} would allocate resources evenly to 
each search space. 
...
\emph{complete} iff it can find out all the answers in finite time, otherwise, 
...
In this section we review the implementation of interleaving depth-first search 
...
The following function, \texttt{conj2}, implements binary conjunction. It applys 
the \emph{first} goal to the input state, then applys the second goal to 
...
definitions introduces syntactic sugars that miniKanren users are more familiar 
...
in the \disj{} macro. We list the new \disj{} with its helpers in 
Fig.~\ref{balanced-disj}. The first helper function, \texttt{split}, takes a 
...
strategy. In contrast, the \disj{} in DFS$_\textrm{i}$ constructs the binary 
tree with the same collection of leaf nodes but in a particularly unbalanced 
form.
