...
\NOTE[LKC]{
We actually have two new BFS implementations. One is modified from DFSf with 
two simple steps. It has been described briefly at the beginning of the BFS 
section but was not given a name. The other is the one described with two large 
figures and was given the name ``our improved BFS''. Both of them runs faster 
than the current BFS.

I didn't name the first one because (1) it is so simple and (2) I considered 
it merely a stepstone to our improved BFS. Now I decided to also give the first 
BFS implementation a name. This is because I think having a nameless impl is 
confusing.

Currently, the nameless implementation is given the name ``our BFS''. And I 
have changed the paper accordingly.

BUT, I am thinking about going further --- removing ``our improved BFS'' from 
...
\NOTE[LKC]{``simpler'' -> ``shorter'' (more objective)}
...
\NOTE[DPF]{...  Mostly the point is to also include a reference to
the two dimitris and to the first OCamren (OCanren), whatever. It is up to you
in this paragraph.  ...}
\NOTE[LKC]{I have fixed the references to TRS and TRS2.}
\NOTE[LKC]{I have cited OCanren in this paragraph.}
...
\NOTE[LKC]{Yes, it is reverso instead of reverseo in Dmitris' paper.}
...
``We never use \texttt{run*} in this paper because fairness is only interesting 
when we have an unbounded number of answers. However, it is perfectly fine to 
use run* with any search strategies.''}
\NOTE[LKC]{I initially wrote it down as a note. I might get it wrong, but I 
thought you suggested me to move it into the paper.}
...
it is not very clear how we manage cost information --- the \texttt{car}s of a 
...
\NOTE[LKC]{I guess you are talking about Table 1 becasue there is no Fig 7. I 
have change the first reference.}
...
may be others.}
\NOTE[LKC]{I have checked all uses of similar macros by text search.}
...
\NOTE[LKC]{I ref the next section instead of the table}
...
\NOTE[LKC]{I guess you are talking about ``our improved BFS''. I have added 
relevant macros.}

\NOTE[LKC]{I rewrote the following paragraph and the table to include our BFS 
in additional to our improved BFS.}
\NOTE[LKC]{I also re-measured all running time. I used to run benchmarks with 
DrRacket, now I do it in command-line. Thus, the time measurements are smaller 
and more stable.}
