...
\NOTE[LKC]{BUT, I am thinking about going further --- removing ``our improved'' from 
...
\NOTE[DPF]{their BFS (or shortened to BFS). We need only say this once if we go with ``our'', which I don't like.
...
\NOTE[LKC]{I mean for all answer in the answer list, the answer's inner lists 
are pairwise \texttt{equal?}. It is impossible to see a \texttt{((a) (b))} in 
the answer list.}
...
this case, we construct a new thunk. The new thunk swaps back the two spaces in the
...
(a$^3$ b$^1$ c$^2$ d$^1$).  If that is not what you mean, you might change the name
to bucket.}
...
\NOTE[LKC]{I have changed the benchmarks about '(I love you), such that their 
relational interpreters doesn't prevent closures from getting into pairs. I did 
so because I consider this restriction doesn't make sense. Benchmarks about 
quine are already very slow so I didn't apply similar changes.}
