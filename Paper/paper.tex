...
\NOTE[DPF]{Throughout the paper we have search spaces and search strategies.
I can see leaving in the  ``search strategies'', but let's try to
remove virtually all of the uses of ``search'' in front of spaces.}
\NOTE[LKC]{Corrected.}

\NOTE[LKC]{BUT, I am thinking about going further --- removing ``our improved'' 
from 
this paper. The first reason is that our BFS is actually more efficient than 
our improved BFS in most benchmarks (see Table 1), despite its theoretical 
space extravagence. The second reason is that DFSi, DFSf, and our BFS look nice 
when sitting together.}

\NOTE[DPF]{Does this mean that the comment about append goes away.
I really thought that was so clever.  I thought that the way you pointed
out the trivial changes was very clear, so that did not bother me at
all.}
\NOTE[LKC]{Yes, the comment about append will go away if we remove our improved 
BFS. Then, let's don't remove it, i.e. have two new BFS implementations at the 
same time.}
\NOTE[LKC]{I think having two new BFSs is a bit confusing. So I brand the 
new and easy/unimproved one as a ``stepping-stone'' and only mention it in BFS 
section.}
...
Using quantitative evaluation, we argue that a new depth-first searches is a
...
\NOTE[DPF]{their BFS (or shortened to BFS). We need only say this once if we go 
with ``our'', which I don't like.
I like ``the new'' or ``the improved'' and ``the current'' }
\NOTE[LKC]{Corrected. I pick ``the improved''.}
...
%BFS implementation'' (``BFS'' for short) as the one by 
%\citet{seres1999algebra} 
%and refer to ``our improved BFS implementation'' (``the improved BFS'' for 
%short) as the new one. We formally prove that \BFSimp{} and \BFSser{} are 
%semantically equivalent, however, \BFSimp{} runs faster in all benchmarks and 
%is shorter.
\end{itemize}

\NOTE[DPF]{The citetseres1999algebra must be not broken:
            You can either rewrite the sentence or
            use a tilde called a tie that means don't break the line.
            Sometimes using tilde does not work out elegant enough,
            so the first idea works best.}
\NOTE[LKC]{I have rewritten the whole item.}
...
\NOTE[DPF]{Can you express exactly how all costs are determined?  I see that
for this simple program, your notion of cost makes sense, but how does it
generalize to all costs.}
\NOTE[LKC]{Corrected.}
...
serves as a ``stepping-stone'' toward \BFSimp{}. It is so similar to \DFSf{} 
...
\NOTE[DPF]{I need clarification: Applying the above changes won't result in 
	``our \BFS,'' but yet a third implementation of \BFS?}
\NOTE[LKC]{If you still need clarification, please add a new note below. 
Otherwise, you might delete these two notes.}

\NOTE[DPF]{Why are the two texttt's macros, again?  Now, it hardly matters,
but it is unfriendly to your other co-authors to have us reading long
expressions when a newcommand would suffice.  I read the paper with a pen
marking it up, then I read the paper on the .tex screen.  That's each time
I read it.}
\NOTE[LKC]{I am sorry about that. Thank you for telling me! I have added many 
more macros and have removed many uses of texttt.}

\NOTE[DPF]{You know that ``bag'' is a technical terms.  It means that there
is a structure where each element of the structure has an associated number, 
which tells you have many occurrences of that element are in the list.
For example, the list (a b a c a d c) could be represented like this:
(a$^3$ b$^1$ c$^2$ d$^1$).  If that is not what you mean, you might change the 
name to bucket.}
\NOTE[LKC]{That is not what I mean. I have changed the name to bucket.}
...
the first buckets of two input spaces. \DFSf{} also has the space extravagance.

\NOTE[LKC]{I rewrite the second last sentence in the above paragraph such that 
it better corresponds to the comment about `append'.}
...
\NOTE[DPF]{Font Space like in the code comments.}
\NOTE[LKC]{corrected}

We make the cost information more clear by changing the \texttt{Space} type, 
...
from a thunk. The remaining functions are as before. We compare these functions 
...
\NOTE[DPF]{ks --> sk and kf --> fk and you don't need to say f and s
mean success continuation and failure continuation.}
\NOTE[LKC]{corrected}
...
no answers, it calls \texttt{fk} with no argument. Otherwise, it calls 
...
result in both \BFSimp{} and BFS. (See supplements for the formal proof.) To 
...
They are also based on a similar test case in \citet{byrd2017unified}. Again, 
...
\NOTE[DPF]{We don't have disequality constraints, so no absento.  You might just
say that.}
\NOTE[LKC]{Corrected.}
...
search strategy. 
Among the two implementations of \BFS, \BFSimp{} constantly perform as well 
or better. 
...
% \BFSimp{}
We proof formally that \BFSimp{} is semantically equivalent to \BFSser{}. But, 
