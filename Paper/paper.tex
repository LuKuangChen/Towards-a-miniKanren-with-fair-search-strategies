...
\NOTE[LKC]{I have restructured the following paragraph.}
...
and \emph{The Reasoned Schemer, 2nd Ed} (TRS2). After \citet{Hemann_2016} 
...
\citet{byrd2017unified} have demonstrated that miniKanren problems that at 
first reading seem impossible are indeed possible by using a Scheme interpreter 
written in miniKanren, thereby having access to logic variables within Scheme.
miniKanren.org contains links to many miniKanren implementations and the seeds 
of many interesting problems and their solutions.

%miniKanren is a family of relational programming languages.
%\citet{Friedman:2005:RS:1121583,friedman_reasoned_2018} introduce
%miniKanren and its implementation in \emph{The Reasoned Schemer} 
%and \emph{The Reasoned Schemer, 2nd Ed} (TRS2). Although originally written in 
%Scheme, miniKanren has been implemented in many other languages, including
%multiple ones using the same language. (See miniKanren.org.) This has come
%about because of \citet{Hemann_2016}'s work on microKanren, a core of 
%miniKanren comprised of only 54 LOC. Also, miniKanren
%has been ported to Ocaml, and named OCanren\citep{kosarev2018typed}). 
%\citet{byrd2017unified} have 
%demonstrated that miniKanren problems that at first reading seem impossible
%are indeed possible by using a Scheme interpreter written in miniKanren, 
%thereby
%having access to logic variables within Scheme.
%miniKanren.org contains the seeds of many interesting problems and 
%their solutions.

\NOTE[DPF]{I thought we agreed to use BFSser throughout?  
In that sense it had emerged a few days ago, and now it has
re-emerged.  It is what we are going to use instead of ``current''.}
\NOTE[LKC]{I am confused. Do you mean we are going to use
	``BFS'' for the search strategy,
	``BFSser'' for Seres's BFS implementation, and
	``the improved BFS'' for our BFS implementation
?}
...
\NOTE[LKC]{I have cited Boskin in the above paragraph.}
...
\NOTE[LKC]{Following Boskin's suggestion, ``are ended with a thunk'' -> ``end 
with thunks''}
...
\NOTE[LKC]{I have rewritten the sentence about \texttt{\#{}f}.}
...
\NOTE[LKC]{(following Boskin's suggestion) constantly -> consistently}
...
have just a few benchmarks, but our benchmarks cover some practically 
interesting cases (e.g. synthesizing expressions that evaluate to \texttt{'(I 
love you)}.
