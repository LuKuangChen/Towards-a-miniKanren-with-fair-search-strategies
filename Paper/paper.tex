...
\NOTE[DPF]{``current'' was for BFSser}
\NOTE[LKC]{Do you mean I should not reserve ``current'' for BFSser, or I should 
do so?}
...
\NOTE[LKC]{I delete the ending sentence of the above paragraph.}
...
result in both BFS and \BFSimp{}. (See supplements for the formal proof.) To 
...
\NOTE[DPF]{Make another macro or use a number when we want to drop ``the''
from ``the improved BFS''.  In the table below, use that macro.}
\NOTE[LKC]{I have defined another macro BFSim, which doesn't start with 
``the''. Also, I have corrected the table below.}
...
\begin{table}
	...
	\begin{tabular}{|c|c|c|c|c|c|c|}
		\hline 
	benchmark & size & \DFSi & \DFSbi & \DFSf & \BFSim & \BFSser	\\
		\hline
...
\NOTE[DPF]{tilde problem 7 lines down.}
\NOTE[LKC]{Corrected.}
...
\NOTE[DPF]{Please leave ``prove'' and don't use ``proof''.}
\NOTE[LKC]{Do you mean we should not use the word ``proof'' everywhere in this 
paper?}
